...
bool CIETabScriptable::Invoke(NPIdentifier methodName, const NPVariant *args, uint32_t argCount, NPVariant *result)
{
	...
	else if(!strcmp(name, "navigate"))
	{
		...
		if(argCount>0 && m_pWebBrowser->GetSafeHwnd() )	{
			CComBSTR bstrUrl(args[0].value.stringValue.utf8length, args[0].value.stringValue.utf8characters);
...
bool CIETabScriptable::GetProperty(NPIdentifier propertyName, NPVariant *result)
{
	...
	else if(!strcmp(name, "hostPtr"))
  {
		if( m_pWebBrowser->GetSafeHwnd() )
      SET_VAR_INT32(result, (int)m_pWebBrowser->get_Application())
  }
...
bool CIETabScriptable::requestBeforeNavigate( LPCWSTR url )
{
	if(!m_pInstance || !m_pRequestTarget)
		return false;

    USES_CONVERSION;
    NPIdentifier methodName = NPN_GetStringIdentifier("beforeNavigate");
    NPVariant arg, result;
    STRINGZ_TO_NPVARIANT(W2UTF8(url), arg);

	NPN_Invoke(m_pInstance->getInstance(), m_pRequestTarget, methodName, &arg, 1, &result);
  return result.value.boolValue;
}

bool CIETabScriptable::requestChangeUrl( LPCWSTR url )
{
	if(!m_pInstance || !m_pRequestTarget)
		return false;

    USES_CONVERSION;
    NPIdentifier methodName = NPN_GetStringIdentifier("changeUrl");
    NPVariant arg, result;
    STRINGZ_TO_NPVARIANT(W2UTF8(url), arg);
