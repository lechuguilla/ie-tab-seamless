...
BEGIN_EVENTSINK_MAP(CIEDlg, CDialog)
  ...
  ON_EVENT(CIEDlg, IDC_IE, DISPID_NEWWINDOW3, OnNewWindow3, VTS_PDISPATCH VTS_PBOOL VTS_I4 VTS_BSTR VTS_BSTR)
...
void CIEDlg::OnNewWindow3(LPDISPATCH* ppDisp,BOOL* Cancel,unsigned long dwFlags, LPCTSTR bstrUrlContext,LPCTSTR bstrUrl)
{
	CIETabScriptable* pObj = m_PluginInst->getScriptableObject();
	if( pObj )
	{
		*Cancel = TRUE;
		int lengthOfMbs = strlen(bstrUrl);
		int wideSize = MultiByteToWideChar(CP_ACP,0,bstrUrl,-1,NULL,0);
		wchar_t* mbs = new wchar_t[wideSize];
		MultiByteToWideChar(CP_ACP, 0, bstrUrl, -1, mbs, wideSize); 
		pObj->requestNewTab(mbs);
		delete []mbs;
...
{
	if (m_PluginInst) {
		const char* scriptStr = "javascript:document.title='IE - '+document.getElementById(\'IECore\').title;";
...
{
	CIETabScriptable* pObj = m_PluginInst->getScriptableObject();
	if (pDisp == m_WB.get_Application() && wcscmp((LPCWSTR)URL->bstrVal, L"about:blank") != 0 ) {
		CIETabScriptable* pObj = m_PluginInst->getScriptableObject();
		//pObj = getScriptableObject();
		if(pObj) {

			pObj->requestChangeUrl(URL->bstrVal);

			//CString test (V_BSTR(PostData));
			//CString sValue(PostData->bstrVal);
			//ULONG bl = PostData->parray->cbElements;
			//UINT a = ::SafeArrayGetElemsize(PostData->parray);
			//UINT a4 = ::SafeArrayGetDim(PostData->parray);
			...
			//if (PostData->parray->cDims == 0) {
			//	// *Cancel = pObj->requestBeforeNavigate((LPCWSTR)URL->bstrVal);
			//}
		}
	}
}
