   ...
   if( aCreateDataStruct->mode==NP_EMBED )
   {
      for( int i=0; i < aCreateDataStruct->argc; ++i )
      {
		  if( 0 == _strcmpi( "URL", aCreateDataStruct->argn[i] ) ) {
			  m_URL = (char*)malloc(strlen(aCreateDataStruct->argv[i]) + 1); 
              strcpy(m_URL, aCreateDataStruct->argv[i]);
		  }
      }
   }
}
...
NPBool nsPluginInstance::init(NPWindow* aWindow)
{
  ...
  m_pIETab = new CIETab(mhWnd, NULL);
  ...
  CString strUrl = CString(m_URL);

	if( strUrl && *strUrl )	{
		COleVariant url(strUrl);
		m_pIETab->getWebBrowser()->Navigate2( &url, NULL, NULL, NULL, NULL );
	}
